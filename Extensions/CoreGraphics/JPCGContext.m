...
- (void)main:(JSContext *)context
{
    ...
    context[@"CGContextStrokeRect"]            = ^void(JSValue *c, NSDictionary *cgRectDict) {
        CGRect rect;
        [JPCGGeometry transCGRectStruct:&rect ofDict:cgRectDict];
    ...
    context[@"CGContextAddLines"]              = ^void(JSValue *c, NSArray *points, NSUInteger count){
        ...
        for (int i = 0; i < count; i++) {
            CGPoint point;
            [JPCGGeometry transCGPointStruct:&point ofDict:points[i]];
            pointsArray[i]   = point;
        }
    ...
    context[@"CGContextAddEllipseInRect"]      = ^void(JSValue *c, NSDictionary *rectDict){
        CGRect rect;
        [JPCGGeometry transCGRectStruct:&rect ofDict:rectDict];
    ...
    context[@"CGContextDrawLinearGradient"]    = ^void(JSValue *c,JSValue *gradient, NSDictionary *startPointDict, NSDictionary *endPointDict,int options) {
        CGPoint startPoint;
        CGPoint endPoint;
        [JPCGGeometry transCGPointStruct:&startPoint ofDict:startPointDict];
        [JPCGGeometry transCGPointStruct:&endPoint ofDict:endPointDict];
    ...
    context[@"CGContextSetLineDash"]             = ^void(JSValue *c, CGFloat phase,
                                                         NSArray *lengths, NSUInteger count) {
    ...
    context[@"CGContextSetShadow"]               = ^void(JSValue *c, NSDictionary *offsetDict,
                                                         CGFloat blur) {
        CGSize size;
        [JPCGGeometry transCGSizeStruct:&size ofDict:offsetDict];
    ...
    context[@"test"] = ^void(JSValue *val) {
        NSString *s = [self formatJSToOC:val];
        NSLog(@"%@",s);
    };
}
